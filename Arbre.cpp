//
// Created by S USER on 12/19/2022.
//
#include <iostream>
#include "Arbre.h"
...
// not in Tree
    ...
    if (node->data == value)
    {
        ...
        node->right = remove(node->right, value); // Target node's data is smaller than the given value then search to right
    else node->left = remove(node->left, value);   // Target node's data is greater than the given value then search to left
